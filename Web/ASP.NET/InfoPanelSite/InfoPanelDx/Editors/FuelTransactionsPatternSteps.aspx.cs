...
{
    public partial class FuelTransactionsPatternSteps : System.Web.UI.Page {

        FuelTransactionsPatternStepManager manager;

        ObservableCollection<FuelTransactionsPattern> FuelTransactionsPatternDataSource;
        ObservableCollection<InfoPanelModels.Models.EnumValues> RouteFuelTransactionStepTypeDataSource;
        ObservableCollection<InfoPanelModels.Models.EnumValues> StepBehaviorDataSource;

        protected void Page_Load(object sender, EventArgs e) {
            manager = FuelTransactionsPatternStepManager.DefaultManager;
        ...
        private async Task UpdateData(DevExpress.Web.ASPxGridView gv)
        {
            ...
            RouteFuelTransactionStepTypeDataSource = await EnumsManager.DefaultManager.GetRouteFuelTransactionStepTypesAsync();
            StepBehaviorDataSource = await EnumsManager.DefaultManager.GetStepBehaviorsAsync();
            ...
            ((GridViewDataComboBoxColumn)gv.Columns["StepType"]).PropertiesComboBox.DataSource = RouteFuelTransactionStepTypeDataSource;
            ((GridViewDataComboBoxColumn)gv.Columns["StepBehavior"]).PropertiesComboBox.DataSource = StepBehaviorDataSource;
